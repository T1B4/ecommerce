...
class Products extends Model {
	...
	public function getInfo($id) {
		...
		$sql = "SELECT
		*,
		(select categories.slug from categories where categories.id = products.id_category ) as category_slug
	...
	private function buildWhere($filters) {
		...
		if(!empty($filters['categories'])) {
			$cat = new Categories();
			$all = $cat->getCategoryRelationship($filters['categories']);
			foreach ($all as $key) {
				$all[] = $cat->getCategoryRelationship($key);
			}
			...
			$cat_item = array_unique($cat_item);

			$where[] = "id_category IN ('".implode("','", $cat_item)."')";
		}
	...
	private function bindWhere($filters, &$sql) {
		...
		if(!empty($filters['brands'])) {
			$sql->bindValue(":id_brand", $filters['brands']);
		}
